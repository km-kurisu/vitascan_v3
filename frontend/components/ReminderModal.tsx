...
import { Calendar, Mail, CheckCircle2, X } from 'lucide-react';
...
export default function ReminderModal({ isOpen, onClose, defaultTitle = "VitaScan Blood Test Follow-up" }: ReminderModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await scheduleReminder({
        ...
        title,
        notes,
        scheduled_date: date || new Date().toISOString()
      });
      setResult(res);
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-white border border-slate-200 rounded-2xl max-w-md w-full p-6 relative text-white shadow-2xl">
        <button onClick={onClose} className="absolute top-4 right-4 text-slate-500 hover:text-white">
          <X className="w-5 h-5" />
        </button>

        <h3 className="text-xl font-bold mb-4 flex items-center space-x-2 text-indigo-400">
        ...
        {result ? (
          <div className="space-y-4 text-center">
            ...
            <p className="font-medium text-emerald-400">Reminder Successfully Scheduled!</p>
            <p className="text-sm text-slate-600">An email notification has been sent to <span className="font-semibold text-white">{email}</span>.</p>
            ...
            <button onClick={onClose} className="mt-2 text-sm text-slate-500 hover:text-white">Close</button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input 
                ...
                className="w-full bg-slate-100 border border-slate-200 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500"
              />
            </div>
